  ...
  const parseExcelJson = (jsonRows: any[]) => {
    try {
      ...
      jsonRows.forEach((rowObj: any, index: number) => {
        ...
        const email = getRowVal(rowObj, ['email', 'email_address', 'mail']);
        const batchName = getRowVal(rowObj, ['batch_name', 'batch', 'batchname']) || 'General';
        const course = getRowVal(rowObj, ['course', 'branch', 'stream']) || 'General Course';
        ...
        const feeDueDate = getRowVal(rowObj, ['fee_due_date', 'feeduedate', 'due_date', 'duedate']) || '15';
        ...
        if (!aadhaar || aadhaar.length === 0) {
          invalidRows.push(`Row ${rowIndex}: ${fullName} (Missing Aadhaar)`);
        } else if (seenAadhaars.has(aadhaar)) {
      ...
      if (invalidRows.length > 0) {
        Alert.alert(
          'Aadhaar Validation Warning ⚠️',
          `Found ${invalidRows.length} record(s) missing mandatory Aadhaar numbers:\n\n${invalidRows.slice(0, 3).join('\n')}\n\nPlease update your Excel sheet to include Aadhaar numbers for all students.`
        );
      } else if (duplicateAadhaars.size > 0) {
        Alert.alert(
          'Duplicate Aadhaar Warning ⚠️',
          `The Excel spreadsheet contains duplicate Aadhaar numbers within the file: ${Array.from(duplicateAadhaars).join(', ')}. Each student must have a unique Aadhaar number.`
        );
      } else {
  ...
  const parseCSVContent = (content: string) => {
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (!aadhaar || aadhaar.length === 0) {
          invalidRows.push(`Row ${i}: ${fullName} (Missing Aadhaar)`);
        } else if (seenAadhaars.has(aadhaar)) {
          duplicateAadhaars.add(aadhaar);
        } else {
          seenAadhaars.add(aadhaar);
      ...
      if (invalidRows.length > 0) {
        Alert.alert(
          'Aadhaar Validation Warning ⚠️',
          `Found ${invalidRows.length} record(s) missing mandatory Aadhaar numbers:\n\n${invalidRows.slice(0, 3).join('\n')}\n\nPlease update your CSV file to include Aadhaar numbers for all students.`
        );
      } else if (duplicateAadhaars.size > 0) {
        Alert.alert(
          'Duplicate Aadhaar Warning ⚠️',
          `The CSV contains duplicate Aadhaar numbers within the file: ${Array.from(duplicateAadhaars).join(', ')}. Each student must have a unique Aadhaar number.`
        );
      } else {
        Alert.alert('CSV Read Successfully', `Found ${rows.length} valid student records with mandatory Aadhaar numbers.`);
      }
    } catch (err) {
  ...
  const handleExecuteImport = async () => {
    ...
    try {
      ...
      parsedStudents.forEach((st, idx) => {
        ...
        const cleanAadhaar = String(st.aadhaar_number).trim();
