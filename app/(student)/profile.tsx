...
import { backupStudentChatsAndMedia } from '@/lib/studentBackupService';
          ...
          <View style={styles.card}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => {
                ...
                setTimeout(async () => {
                  try {
                    await backupStudentChatsAndMedia(
                      user.id,
                      studentDetails?.business_id,
                      (step, detail) => {
                        if (step === 'success') {
                          setBackupStep('success');
                          setBackupDetail(detail || 'Chats & study media backed up successfully.');
                        } else if (step === 'failed') {
                          setBackupStep('failed');
                          setBackupDetail(detail || 'Backup failed.');
                        } else {
                          setBackupDetail(detail || 'Processing backup...');
                        }
                      }
                    );
                  } catch (err: any) {
            ...
            >
              ...
              <Text style={styles.menuLabel}>Backup Chats & Study Media</Text>
              <Ionicons name="chevron-forward" size={18} color={Colors.text.tertiary} />
            </TouchableOpacity>
