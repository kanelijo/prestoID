...
export default function AdminProfileScreen() {
  ...
  const handleForceBackup = async () => {
    if (!businessId) {
      CustomAlert.alert('Export Error', 'Coaching institute details not found.');
      return;
    }
    ...
    try {
      const success = await exportAdminCoachingToDrive(
        businessId,
        businessName || 'Coaching Records',
        (step, detail) => {
          if (step === 'success') {
            setBackupStep('success');
            setBackupDetail(detail || 'Coaching records successfully exported to Google Drive.');
            playAudioFeedback('success');
          } else if (step === 'failed') {
            setBackupStep('failed');
            setBackupDetail(detail || 'Export failed.');
          } else {
            setBackupDetail(detail || 'Processing export...');
          }
        }
      );
      ...
      setBackupStep('failed');
      setBackupDetail(err.message || 'Export procedure encountered an error.');
    }
  };
